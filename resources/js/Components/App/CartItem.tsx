          ...
          <div className="flex justify-between items-center mt-4">
            <div className="flex gap-2 items-center">
              ...
              <div className={error ? 'tooltip tooltip-open tooltip-error' : ''} data-tip={error}>
                <TextInput type='number' defaultValue={item.quantity} onChange={handleQuantityChange}
                  className='input-sm w-16'
                />
                <button onClick={() => onDeleteClick()} className='btn btn-ghost btn-sm'>Delete</button>
                <button onClick={()=>{}} className='btn btn-ghost btn-sm'>Save for Later</button>
              </div>
