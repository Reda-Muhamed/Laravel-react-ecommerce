...
export default function MiniCartDropdown() {
  ...
  console.log('my cart items ',miniCartItems)
        ...
        <div className="card-body">
          ...
          <div className="my-4 max-h-[300px] overflow-auto">
            {miniCartItems.length === 0 ? (
              <div className="py-4 text-gray-500 text-center">Your cart is empty</div>
            ) : (
              miniCartItems.map((item, index) => (
                <div key={`${item.id}-${index}`} className="flex gap-4 p-3">
                  <Link href={productRoute(item)} className="w-16 h-16 flex justify-center items-center">
                    <img src={item.image || '/placeholder.png'} alt={item.title} className="max-w-full max-h-full" />
                  </Link>
                  <div className="flex-1">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <div><CurrencyFormatter amount={item.price * item.quantity} /></div>
                    </div>
                  </div>
                </div>
              ))
